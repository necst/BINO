...
if __name__=="__main__":
    ...
    for opt in OPTS:
        table = Table(["Method", "S", "N", "TP", "Rec"], [25, 6, 6, 6, 6])
        print("Known results for optimization %s:" % (opt))
        for method in METHODS:
            n, tp, fp = prjs_res.get_stats(M, S, [method], [opt], True)
            if n == 0:
                rec = 0
            else:
                rec = tp/n
            ...
            table.add_row([method, "%.2f" % S, "%d" % n, "%d" % tp, "%.2f" % rec])
        print(table)
        print("")
        print("")
    for opt in OPTS:
        table = Table(["Method", "S", "N", "TP", "Rec"], [25, 6, 6, 6, 6])
        print("Unknown results for optimization %s:" % (opt))
        for method in METHODS:
            n, tp, fp = prjs_res.get_stats(M, S, [method], [opt], False)
            if n == 0:
                rec = 0
            else:
                rec = tp/n
