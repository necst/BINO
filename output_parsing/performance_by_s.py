...
OPTS += ["-Os"]
METHODS = []
METHODS += ["std::map::operator[]", "std::map::lower_bound", "std::map::upper_bound", "std::map::find"]
METHODS += ["std::vector::push_back", "std::vector::resize", "std::vector::clear","std::vector::reserve","std::vector::erase"]
METHODS += ["std::deque::push_back", "std::deque::pop_front", "std::deque::operator[]"]
...
if __name__=="__main__":
    ...
    for method in METHODS:
        for opt in OPTS:
            table = Table(["Method", "S", "N", "TP", "FP", "Rec", "Prec", "F1-Score"], [25, 6, 6, 6, 6, 6, 6, 10])
            print("Results for %s with %s:" % (method, opt))
            for s in np.arange(0.75, 1.00, 0.01):
                n, tp, fp = prjs_res.get_stats(M, float(s), [method], [opt])
                if n == 0:
                    rec = 0
                else:
                    rec = tp/n
                ...
                table.add_row([method, "%.2f" % s, "%d" % n, "%d" % tp, "%d" % fp, "%.2f" % rec, "%.2f" % prec, "%.2f" % f1])
            global_n += n
            print(table)
            print("")
            print("")