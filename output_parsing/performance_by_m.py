...
if __name__=="__main__":
    ...
    for method in METHODS:
        table = Table(["Method", "S", "M", "N", "TP", "FP", "Rec", "Prec", "F1-Score"], [40, 6, 6, 6, 6, 6, 6, 6, 15])
        for m in range(5, 10):
            for s in list(np.arange(0.75, 1.0, 0.01)):     
                n, tp, fp = prjs_res.get_stats(m, float(s), [method], OPTS)
                if n == 0:
                    rec = 0
                else:
                    rec = tp/n
                ...
                table.add_row([method, "%.2f" % s, "%d" % m, "%d" % n, "%d" % tp, "%d" % fp, "%.2f" % rec, "%.2f" % prec, "%.5f" % f1])
        print(table)
        print("")
        print("")